...
const GameScreen = ({ onBackToMenu }: GameScreenProps) => {
  ...
  const drawBird = useCallback((ctx: CanvasRenderingContext2D, bird: BirdPosition) => {
    ...
    if (bird.bird.name === 'Bee') {
      ...
      renderSize = Math.round(renderSize * 3.8);
    }
    ...
    // Handle direction and hit state (match original transform logic)
  ...
  const gameLoop = useCallback(() => {
    ...
    if (gameStarted && !gameOver) {
      ...
      // Draw all birds from ref
      birdsRef.current.forEach(bird => drawBird(ctx, bird));
      
      // Update/draw floating points from ref
      if (floatingPointsRef.current.length > 0) {
        ...
        floatingPointsRef.current.forEach(point => drawFloatingPoint(ctx, point));
      }
    }
  ...
  }, [gameStarted, gameOver, drawBackground, drawBird, drawFloatingPoint, drawBatCursor]);
  ...
  const createBird = useCallback((flockOptions?: { side: number; y: number }) => {
    ...
    // Keep refs authoritative for RAF
  ...
  const catchBird = useCallback(async (birdId: string) => {
    ...
    if (bird && bird.status === 'flying') {
      ...
      birdsRef.current = birdsRef.current.map(b =>
        b.id === birdId ? { ...b, status: 'hit', velocityX: 0, velocityY: 2 } : b
      );
  ...
  const moveBirds = useCallback(() => {
    ...
    for (const bird of birdsRef.current) {
      if (bird.status === 'hit') {
        ...
        if (newY <= height + 100) {
          updated.push({ ...bird, y: newY, velocityY: newVelocityY });
        }
        continue;
      }
      ...
      if (newX >= -150 && newX <= width + 150 && newY >= -150 && newY <= height + 150) {
        updated.push({ ...bird, x: newX, y: newY });
      }
    }
    birdsRef.current = updated;
    // Mirror into React state less frequently could be added later; keep immediate for now to preserve UI
    // No React state mirroring here; unified RAF/canvas is the display source
  ...
  const playAgain = () => {
    ...
    // Cancel any pending RAFs for movement to avoid duplicate loops

...
const GameScreen = ({ onBackToMenu }: GameScreenProps) => {
  ...
  // Smoothed hand coords (normalized 0..1 in selfie space mapped to container)
  const handNormRef = useRef<{ x: number; y: number } | null>(null);
  const handSmoothedRef = useRef<{ x: number; y: number } | null>(null);

  // Fixed mapping parameters (no UI): palm-based control with modest smoothing
  const handParams = { smoothing: 0.35 } as const;
  ...
  // Map normalized hand coords to container space with smoothing
  useEffect(() => {
    if (!handModeEnabled) return;
    let raf: number;
    const tick = () => {
      if (gameContainerRef.current && handSmoothedRef.current) {
        const rect = gameContainerRef.current.getBoundingClientRect();
        const targetX = handSmoothedRef.current.x * rect.width;
        const targetY = handSmoothedRef.current.y * rect.height;
        // Update ref-driven DOM position to avoid full re-render cost
        if (batRef.current) {
          batRef.current.style.left = `${targetX}px`;
          batRef.current.style.top = `${targetY}px`;
          batRef.current.style.transform = 'translate(-50%, -50%)';
        }
        // Keep state in sync for HUDs/logic
  ...
  return (
    ...
    <div className="min-h-screen w-full bg-background text-foreground font-press-start flex flex-col items-center justify-start p-8 relative select-none">
      ...
      <HandTrackingView
        ...
        onPinch={() => {
          // Trigger the same hit animation and click logic centered at current bat position
          setCursor(prev => ({ ...prev, isHitting: true, hitFrame: 0, hitTimer: 0 }));
          if (gunAudioRef.current) {
            gunAudioRef.current.currentTime = 0;
            gunAudioRef.current.play();
          }
          if (gameContainerRef.current && lastPalmRef.current) {
            const rect = gameContainerRef.current.getBoundingClientRect();
            const cx = lastPalmRef.current.x * rect.width;
            const cy = lastPalmRef.current.y * rect.height;
            const hitRadius = getScaledSize(24);
            birds.forEach(bird => {
              if (bird.status === 'flying') {
        ...
        onHandData={(data) => {
          if (!handModeEnabled) return;
          const source = data.palmCenter; // control from palm only
          if (!source) return;

          // No offsets, no fist compensation. Keep simple and stable.
          let nx = source.x;
          let ny = source.y;
          nx = Math.max(0, Math.min(1, nx));
          ny = Math.max(0, Math.min(1, ny));

          lastPalmRef.current = { x: nx, y: ny };

          const alpha = handParams.smoothing;
          const prev = handNormRef.current ?? { x: nx, y: ny };
          const smoothed = {
            x: prev.x + alpha * (nx - prev.x),
            y: prev.y + alpha * (ny - prev.y),
          };
          handNormRef.current = smoothed;
          handSmoothedRef.current = smoothed;
        }}
      />

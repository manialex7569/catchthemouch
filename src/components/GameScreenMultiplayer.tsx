  ...
  return (
    ...
    <div className="min-h-screen w-full bg-background text-foreground font-press-start flex flex-col items-center justify-between p-8 relative select-none">
      ...
      <HandTrackingView
        ...
        onPinch={() => {
          // Trigger hit using same-frame palm position
          setCursor(prev => ({ ...prev, isHitting: true, hitFrame: 0, hitTimer: 0 }));
          if (gunAudioRef.current) { gunAudioRef.current.currentTime = 0; gunAudioRef.current.play(); }
          if (gameContainerRef.current && lastPalmRef.current && gameStarted && !gameOver && !waitingForPlayers) {
            const rect = gameContainerRef.current.getBoundingClientRect();
        ...
        onHandData={(data) => {
          ...
          if (!source) return;
          let nx = Math.max(0, Math.min(1, source.x));
          let ny = Math.max(0, Math.min(1, source.y));
          lastPalmRef.current = { x: nx, y: ny };
          const alpha = handParams.smoothing;
          const prev = handNormRef.current ?? { x: nx, y: ny };
          const smoothed = { x: prev.x + alpha * (nx - prev.x), y: prev.y + alpha * (ny - prev.y) };

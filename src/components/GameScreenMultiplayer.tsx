...
const GameScreenMultiplayer = ({ onBackToMenu, isHost, roomId }: GameScreenMultiplayerProps) => {
  ...
  const gameLoop = useCallback(() => {
    ...
    if (gameStarted && !gameOver && !waitingForPlayers) {
      ...
      if (floatingPointsRef.current.length > 0) {
        for (let i = 0; i < floatingPointsRef.current.length; i++) { const p = floatingPointsRef.current[i]; p.y -= 4; p.opacity -= 0.04; }
        floatingPointsRef.current = floatingPointsRef.current.filter(p => p.opacity > 0);
        floatingPointsRef.current.forEach(p => drawFloatingPoint(ctx, p));
      }
    }
    drawBatCursor(ctx);
    gameLoopRef.current = requestAnimationFrame(gameLoop);
  }, [gameStarted, gameOver, waitingForPlayers, drawBackground, drawBird, drawFloatingPoint, drawBatCursor]);
  ...
  const moveBirds = useCallback(() => {
    if (gameOver || !gameStarted || waitingForPlayers || !gameContainerRef.current) return;
  ...
  }, [gameStarted, gameOver, catchBird, getScaledSize]);

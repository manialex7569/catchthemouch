  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-background text-foreground font-press-start p-4">
      ...
      <div className="w-full max-w-5xl mb-4">
        <div className="flex items-center justify-between mb-4">
          <Button
            onClick={onBack}
            variant="outline"
            className="font-press-start flex items-center gap-2"
          >
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="bg-card border rounded-lg p-4 sticky top-4">
              ...
              <div className="space-y-3">
                <div className="text-center">
                  ...
                  <p className="text-xl font-bold text-primary">{playerStats.totalScore}</p>
                </div>
                <div className="text-center">
                  <p className="text-sm text-muted-foreground">Total Hits</p>
                  <p className="text-xl font-bold text-secondary">{playerStats.hitCount}</p>
                </div>
                <div className="text-center">
                  <p className="text-sm text-muted-foreground">Address</p>
                  <p className="text-xs font-mono">{formatAddress(playerStats.player)}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className={`bg-card border rounded-lg p-4 ${playerStats ? 'lg:col-span-2' : 'lg:col-span-3'}`}>
          ...
          {topScores.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground">Start playing to see your score here!</p>
            </div>
          ) : (
              ...
              {topScores.map((score, index) => (
                <div
                  ...
                  className={`flex items-center justify-between p-2 rounded-lg border ${
                    address === score.player ? 'bg-primary/10 border-primary' : 'bg-background'
                  }`}
                >
                    ...
                    <div>
                      <p className="font-bold text-sm">
                        ...
                        {address === score.player && (
                          <span className="ml-2 text-xs bg-primary text-primary-foreground px-1 py-0.5 rounded">
                            YOU
                          </span>
                        )}
                      </p>
